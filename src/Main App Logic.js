...
// Safely handle the environment variable for the preview context
...
const Dashboard = ({ user, activeCarnival, daysLeft, stats, priorities, setPriorities }) => {
  ...
  useEffect(() => { setTempPriorities(priorities); }, [priorities]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Hero Card */}
      <div className={`bg-gradient-to-r ${gradients[activeCarnival.color] || gradients.teal} rounded-2xl p-6 text-white shadow-lg relative overflow-hidden`}>
        <div className="relative z-10">
          <h2 className="text-xs font-medium uppercase tracking-wider opacity-90 flex items-center gap-1">
            <Clock size={14} className="opacity-90" /> Countdown to {activeCarnival.name}
          </h2>
          <div className="flex items-baseline mt-2">
            <span className="text-5xl font-extrabold leading-none">{daysLeft}</span>
            <span className="ml-2 text-xl opacity-90">days left</span>
          </div>
          <p className="mt-3 text-xs font-light italic opacity-90">
            {activeCarnival.country} • {new Date(activeCarnival.date).toDateString()}
          </p>
        </div>
        <div className="absolute -right-4 -bottom-8 w-32 h-32 bg-white opacity-10 rounded-full blur-xl"></div>
      </div>
      ...
      <div className="bg-white border-l-4 border-slate-800 rounded-r-xl p-4 shadow-sm flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">{activeCarnival.ad.text}</p>
        </div>
          ...
          {tempPriorities.map((p, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              {editMode ? (
                ...
                <span className={`text-sm ${!p ? 'text-slate-400 italic' : 'text-slate-700'}`}>
                  {p || 'Tap edit to add...'}
                </span>
              )}
            </div>
...
const Budget = ({ items, onAdd, onDelete, color }) => {
  ...
  return (
    <div className="space-y-4 pb-20">
      <div className={`bg-${color}-50 p-4 rounded-xl border border-${color}-100 flex justify-between items-center`}>
        <div>
          <p className={`text-xs font-bold text-${color}-600 uppercase`}>Total Budget</p>
          <p className={`text-xl font-black text-${color}-800`}>{formatCurrency(total)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-500">Balance Due</p>
          <p className="text-lg font-bold text-slate-700">{formatCurrency(total - paid)}</p>
        </div>
      </div>

      <div className="space-y-2">
        {items.map(item => (
          <div key={item.id} className="bg-white p-3 rounded-lg shadow-sm border border-slate-100 flex justify-between items-center">
            <div>
              <p className="text-sm font-semibold text-slate-800">{item.category}</p>
              <p className="text-xs text-slate-500">Paid: {formatCurrency(item.paid)} <span className="text-slate-300">|</span> Due: <span className="text-orange-600">{formatCurrency(item.amount - item.paid)}</span></p>
            </div>
      ...
      <button onClick={() => setIsAdding(true)} className={`fixed bottom-20 right-4 p-4 rounded-full shadow-lg bg-${color}-600 text-white`}>
        <Plus size={24} />
      </button>
      ...
      {isAdding && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
           <div className="bg-white w-full rounded-xl p-4 space-y-3">
             ...
             <input className="w-full p-2 border rounded" placeholder="Category" value={newItem.category} onChange={e=>setNewItem({...newItem, category:e.target.value})} />
             <div className="flex gap-2">
               <input className="w-1/2 p-2 border rounded" type="number" placeholder="Cost" value={newItem.amount} onChange={e=>setNewItem({...newItem, amount:e.target.value})} />
               <input className="w-1/2 p-2 border rounded" type="number" placeholder="Paid" value={newItem.paid} onChange={e=>setNewItem({...newItem, paid:e.target.value})} />
             </div>
             <button onClick={handleAdd} className={`w-full py-3 rounded-lg font-bold text-white bg-${color}-600`}>Add</button>
             <button onClick={()=>setIsAdding(false)} className="w-full py-2 text-slate-500">Cancel</button>
           </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center">
        ...
        <span className={`text-xs bg-${color}-50 text-${color}-700 px-2 py-1 rounded-full font-bold`}>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 divide-y divide-slate-50">
        {items.map(item => (
          <div key={item.id} className="p-4 flex items-center justify-between group">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => onToggle(item.id)}>
              <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${item.checked ? `bg-${color}-500 border-${color}-500` : 'border-slate-300'}`}>
                {item.checked && <CheckSquare size={14} className="text-white" />}
              </div>
              <span className={`text-sm ${item.checked ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{item.item}</span>
            </div>
            {/* Affiliate Link Button */}
            {item.affiliate && (
              <a href={item.affiliate} target="_blank" rel="noopener noreferrer" className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded flex items-center gap-1">
...
const Schedule = ({ events, onAdd, onDelete, color, activeCarnival }) => {
  ...
  const [isAdding, setIsAdding] = useState(false);
  const [newEvent, setNewEvent] = useState({ title: '', time: '', type: 'fete' });

  // Generate dynamic dates based on the active carnival
  const dates = useMemo(() => getCarnivalDates(activeCarnival.date), [activeCarnival]);
  
  useEffect(() => {
    if (dates.length > 0 && !selectedDate) setSelectedDate(dates[2].date); // Default to a middle day
  }, [dates, selectedDate]);
    ...
    <div className="flex flex-col h-full space-y-4">
          ...
          <button
            ...
            className={`flex-shrink-0 flex flex-col items-center justify-center w-14 h-16 rounded-xl transition-all border ${
              selectedDate === day.date 
                ...
                : day.isMain ? `bg-${color}-50 text-${color}-800 border-${color}-200` : 'bg-white text-slate-500 border-slate-100'
            }`}
          >
            <span className="text-[10px] font-bold uppercase">{day.label.split(' ')[0]}</span>
      ...
      <div className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-slate-100 overflow-hidden flex flex-col">
        <div className="mb-4 flex items-center justify-between">
           ...
           <span className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-500">{filteredEvents.length} Events</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pb-20">
          {filteredEvents.map(event => (
            <div key={event.id} className="flex gap-3 group">
              ...
              <div className={`flex-1 p-3 rounded-lg border-l-4 shadow-sm relative ${
                event.type === 'fete' ? 'bg-purple-50 border-purple-400' : 
                event.type === 'mas' ? `bg-${color}-50 border-${color}-400` : 'bg-slate-50 border-slate-400'
              }`}>
                <div className="flex justify-between items-start">
                  <span className="font-semibold text-slate-700 text-sm">{event.title}</span>
                  <button onClick={() => onDelete(event.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={14}/></button>
                </div>
                <span className="text-[10px] uppercase tracking-wide text-slate-400 mt-1 block">{event.type}</span>
              </div>
            </div>
          ))}
          {filteredEvents.length === 0 && (
            <div className="text-center py-8 text-slate-400 text-sm italic">
              No plans for this day.<br/>Tap + to add a fete.
            </div>
          )}
        </div>
      </div>
      ...
      {isAdding && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-white w-full rounded-xl p-4 space-y-4 animate-in slide-in-from-bottom-10">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => setIsAdding(false)} className="flex-1 py-2 text-slate-500 font-medium text-sm">Cancel</button>
              <button onClick={handleAdd} className={`flex-1 py-2 bg-${color}-600 text-white rounded-lg font-bold text-sm`}>Save</button>
            </div>
          </div>
        </div>
      )}
...
const Vault = ({ logistics, color }) => (
  <div className="space-y-4">
    ...
    <button className={`w-full py-3 rounded-xl border-2 border-dashed border-${color}-200 text-${color}-600 font-bold text-sm flex items-center justify-center gap-2`}>
...
export default function App() {
  ...
  const daysLeft = useMemo(() => getDaysUntil(activeCarnival.date), [activeCarnivalId]);
  ...
  useEffect(() => {
    ...
    const unsubBudget = onSnapshot(collection(db, ...getPath(uid, activeCarnivalId, 'budget')), 
      s => setBudgetItems(s.docs.map(d => ({id:d.id, ...d.data()})))
    );
    const unsubPacking = onSnapshot(collection(db, ...getPath(uid, activeCarnivalId, 'packing')), 
      async s => {
        if(s.empty) {
    ...
    const unsubEvents = onSnapshot(collection(db, ...getPath(uid, activeCarnivalId, 'events')), 
      s => setEvents(s.docs.map(d => ({id:d.id, ...d.data()})))
    );
    // Fix: Ensure priority list is an array and filter out non-strings to avoid React object rendering errors
    const unsubPriorities = onSnapshot(doc(db, ...getPath(uid, activeCarnivalId, 'meta'), 'priorities'), 
      s => { 
        if(s.exists()) {
    ...
    return () => { unsubBudget(); unsubPacking(); unsubEvents(); unsubPriorities(); };
  ...
  const togglePacking = async (id) => {
    ...
    if(item) await setDoc(doc(db, ...getPath(user.uid, activeCarnivalId, 'packing'), id), {...item, checked: !item.checked});
  };
  const savePriority = async (list) => {
    setPriorities(list);
    if(user) await setDoc(doc(db, ...getPath(user.uid, activeCarnivalId, 'meta'), 'priorities'), {list});
  };

  if (!user) return <div className="h-screen flex items-center justify-center text-slate-400 text-sm">Loading...</div>;
  ...
  return (
    <div className="bg-slate-50 h-screen w-full font-sans flex flex-col max-w-md mx-auto shadow-2xl relative overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 scrollbar-hide">
        {activeTab === TABS.DASHBOARD && <Dashboard user={user} activeCarnival={activeCarnival} daysLeft={daysLeft} stats={stats} priorities={priorities} setPriorities={savePriority} />}
        {activeTab === TABS.BUDGET && <Budget items={budgetItems} onAdd={addBudgetItem} onDelete={deleteBudgetItem} color={activeCarnival.color} />}
        {activeTab === TABS.PACKING && <Packing items={packingList} onToggle={togglePacking} color={activeCarnival.color} />}
        {activeTab === TABS.SCHEDULE && <Schedule events={events} onAdd={addEvent} onDelete={deleteEvent} color={activeCarnival.color} activeCarnival={activeCarnival} />}
        {activeTab === TABS.VAULT && <Vault logistics={{}} color={activeCarnival.color} />}
      </main>
