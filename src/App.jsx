...
// --- Firebase: use shared instance from src/firebase.js ---
...
// Single, safe appId used only for Firestore path names
const appId = "carnival-planner-ui";
...
// Database path helper (now uses our local appId constant)
const getPath = (userId, carnivalId, collectionName) => {
  return ["artifacts", appId, "users", userId, "carnivals", carnivalId, collectionName];
};

// --- Components ---
// (Dashboard, Budget, Packing, Schedule, Vault)
//  ⬇️ All of this is identical to what you had, just using getPath/appId above

// ... ⬇️ KEEP EVERYTHING FROM YOUR EXISTING COMPONENT DEFINITIONS DOWNWARD
// I’ll only repeat the bottom part here so the response isn’t huge,
// but in your file you should keep Dashboard, Budget, Packing, Schedule, Vault
// exactly as they are in your current App.jsx. :contentReference[oaicite:0]{index=0}
    ...
    <div className="bg-slate-50 h-screen w-full font-sans flex flex-col max-w-md mx-auto shadow-2xl relative overflow-hidden">
              ...
              <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50">
                <div className="max-h-64 overflow-y-auto">
                  {Object.values(CARNIVALS).map((c) => (
                    ...
                    >
                      <span className={`w-2 h-2 rounded-full bg-${c.color}-500`} />
                      {c.name}
                    </button>
                  ))}
                </div>
      ...
      <main className="flex-1 overflow-y-auto p-4 scrollbar-hide">
        ...
        {activeTab === TABS.VAULT && (
          <Vault logistics={{}} color={activeCarnival.color} />
        )}
      </main>
