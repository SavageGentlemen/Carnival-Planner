...
function CreatorRequests({ user }) {
    ...
    const handleAction = async (id, status) => {
        if (!confirm(`Are you sure you want to ${status} this creator?`)) return;
        try {
            ...
            // Also update user profile to reflect creator status if approved
            if (status === 'approved') {
                // Try to update user profile if it exists, or user doc
                        ...
                        <div key={req.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div>
                                ...
                                <div className="flex gap-2 text-xs">
                                    <span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded">Code: {req.affiliateCode}</span>
                                    <span className="text-gray-400">Applied: {req.appliedAt?.toDate().toLocaleDateString()}</span>
                                </div>
                            </div>
                            <div className="flex gap-2 w-full md:w-auto">
                                <button
                                    onClick={() => handleAction(req.id, 'rejected')}
                                    className="flex-1 md:flex-none px-4 py-2 border border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition-colors flex items-center justify-center gap-2"
                                >
