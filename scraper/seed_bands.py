...
def init_firebase():
    ...
    firebase_admin.initialize_app(cred, {'projectId': 'carnival-planner'})
    return firestore.client(database_id='squad-db')

def seed_bands():
    print(f"Seeding {len(BANDS)} bands...")
    db = init_firebase()
    batch = db.batch()
    
    for band in BANDS:
        # Create a unique ID based on name and carnival
        unique_id = f"{band['carnivalId']}-{band['name'].lower().replace(' ', '-')}"
        doc_ref = db.collection('bands').document(unique_id)
        
        band['updatedAt'] = datetime.datetime.now()
        batch.set(doc_ref, band, merge=True)
