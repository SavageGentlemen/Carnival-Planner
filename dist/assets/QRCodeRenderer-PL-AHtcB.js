...
function QRCodeRenderer({ ecl = "M", size: sizeProp = 200, uri, clearSize = 0, image, imageBackground = "transparent" }) {
  ...
  const dots = reactExports.useMemo(() => {
    ...
    for (const { x, y } of qrList) {
      ...
      for (let i = 0; i < 3; i++) {
        dotsArray.push(jsxRuntimeExports.jsx("rect", { fill: i % 2 !== 0 ? "var(--ck-qr-background, var(--ck-body-background))" : "var(--ck-qr-dot-color)", height: cellSize * (7 - i * 2), rx: (i - 2) * -5 + (i === 0 ? 2 : 3), ry: (i - 2) * -5 + (i === 0 ? 2 : 3), width: cellSize * (7 - i * 2), x: x1 + cellSize * i, y: y1 + cellSize * i }, `${i}-${x}-${y}`));
      }
    }
    if (image) {
      const x1 = (matrix.length - 7) * cellSize * 1;
      const y1 = (matrix.length - 7) * cellSize * 1;
      dotsArray.push(jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [jsxRuntimeExports.jsx("rect", { fill: imageBackground, height: cellSize * (7 - 0 * 2), rx: (0 - 2) * -5 + 2, ry: (0 - 2) * -5 + 2, width: cellSize * (7 - 0 * 2), x: x1 + cellSize * 0, y: y1 + cellSize * 0 }), jsxRuntimeExports.jsx("foreignObject", { height: cellSize * (7 - 0 * 2), width: cellSize * (7 - 0 * 2), x: x1 + cellSize * 0, y: y1 + cellSize * 0, children: jsxRuntimeExports.jsx("div", { style: { borderRadius: (0 - 2) * -5 + 2, overflow: "hidden" }, children: image }) })] }));
    }
        ...
        if (matrix[i]?.[j]) {
          if (!(i < 7 && j < 7 || i > matrix.length - 8 && j < 7 || i < 7 && j > matrix.length - 8)) {
            if (image || !(i > matrixMiddleStart && i < matrixMiddleEnd && j > matrixMiddleStart && j < matrixMiddleEnd)) {
              dotsArray.push(jsxRuntimeExports.jsx("circle", { cx: i * cellSize + cellSize / 2, cy: j * cellSize + cellSize / 2, fill: "var(--ck-qr-dot-color)", r: cellSize / 3 }, `circle-${i}-${// biome-ignore lint/suspicious/noArrayIndexKey: its the only thing available for the key here
              j}`));
            }
          }
        }
  ...
  return jsxRuntimeExports.jsxs("svg", { height: size, role: "presentation", style: {
    height: size,
    width: size
  }, viewBox: `0 0 ${size} ${size}`, width: size, children: [jsxRuntimeExports.jsx("rect", { fill: "transparent", height: size, width: size }), dots] });
}
export {
  QRCodeRenderer as default
};
