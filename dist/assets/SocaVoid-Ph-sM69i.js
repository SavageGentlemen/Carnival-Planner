...
function createFeather(w, h) {
  ...
  return {
    ...
    rotationSpeed: rand(-8e-3, 8e-3),
    rotationY: rand(0, Math.PI * 2),
    rotationYSpeed: rand(-6e-3, 6e-3),
    rotationZ: rand(0, Math.PI * 2),
    rotationZSpeed: rand(-4e-3, 4e-3),
    color,
...
function drawNebula(ctx, w, h, time) {
  ...
  clouds.forEach((c) => {
    ...
    const cx = c.x + Math.sin(time * 2e-4 * 60 + c.phase) * 30;
    const cy = c.y + Math.cos(time * 15e-5 * 60 + c.phase) * 20;
...
function SocaVoid() {
  ...
  const initParticles = reactExports.useCallback((w, h) => {
    const p = [];
  ...
  const updateParticle = reactExports.useCallback((p, w, h, mouse, clicking, clickCenter, dt) => {
    ...
    p.x += p.vx * dt;
    p.y += p.vy * dt;
    if (p.rotation !== void 0) p.rotation += (p.rotationSpeed || 0) * dt;
    if (p.rotationY !== void 0) p.rotationY += (p.rotationYSpeed || 0) * dt;
    if (p.rotationZ !== void 0) p.rotationZ += (p.rotationZSpeed || 0) * dt;
    if (p.flipAngle !== void 0) p.flipAngle += (p.flipSpeed || 0) * dt;
    if (p.phase !== void 0) p.phase += (p.phaseSpeed || 0) * dt;
    const pad = 60;
