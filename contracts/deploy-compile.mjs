 ...
 * Downloads solc on-the-fly — no Hardhat or devDependencies needed.
 *
 * Usage:
 *   DEPLOYER_PRIVATE_KEY=0x... node contracts/deploy-compile.mjs
 *   DEPLOYER_PRIVATE_KEY=0x... NETWORK=mainnet node contracts/deploy-compile.mjs
 */
...
function loadSolc() {
    try {
        ...
        return require2('solc');
    }
}

// ── Compile Contract ──
function compileContract(solc) {
    const contractPath = join(__dirname, 'CarnivalPassport.sol');
...
async function main() {
    ...
    if (!privateKey) {
        ...
        console.error('   Example: DEPLOYER_PRIVATE_KEY=0x... node contracts/deploy-compile.mjs');
        process.exit(1);
    }
    ...
    if (balance === 0n) {
        ...
        console.error('   https://www.base.org/faucet');
    ...
    console.log(`  1. echo 'VITE_PASSPORT_CONTRACT=${address}' >> .env`);
    console.log(`  2. firebase functions:secrets:set WEB3_CONTRACT_ADDRESS`);
    console.log(`     (paste: ${address})`);
    console.log(`  3. firebase functions:secrets:set WEB3_PRIVATE_KEY`);
    console.log(`     (paste your deployer private key)`);
    console.log(`  4. firebase functions:secrets:set WEB3_RPC_URL`);
    console.log(`     (paste: ${network.rpc})`);
